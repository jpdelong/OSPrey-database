...
#=
df_dens_rodents = df_density[findall(df_density.Order .== "Rodentia" .|| df_density.Order .== "Eulipotyphla" .|| df_density.Order .== "Lagomorpha" .|| df_density.Order .== "Dasyuromorphia" .|| df_density.Order .== "Didelphimorphia"),:] # drop everything that is rows with just NA
unique(df_dens_rodents.Order)
df_dens_rodents[!, :BodyMass] .= masses
df_dens_rodents = df_dens_rodents[findall(df_dens_rodents.BodyMass .!= 0),:] # drop rows with 0 body mass
df_dens_rodents[!, :LogDensity] = log10.(df_dens_rodents.Density_km) # drop rows with 0 body mass
df_dens_rodents[!, :LogMass] = log10.(df_dens_rodents.BodyMass) # drop rows with 0 body mass
lm_1 = lm(@formula(LogDensity ~ LogMass * Order), df_dens_rodents)
lm_2 = lm(@formula(LogDensity ~ LogMass + Order), df_dens_rodents)
mainr2 = r2(lm_1)
=#
...
f_ranks = Figure()
    ...
    #ax_mass = Axis(f_ranks[1, 2], xlabel = "Rank of body mass", ylabel = "Rank in diet")

    ax_hist_dens = Axis(f_ranks[1, 2], xlabel = "Slope of line", ylabel = "Frequency")
    #ax_hist_mass = Axis(f_ranks[2, 2], xlabel = "Slope of line", ylabel = "Frequency")
    #rowsize!(f_ranks.layout, 1, Aspect(1, 1.0))
# =============================================================================
...
mass_slopes = Array{Union{Float64,Missing}}(missing, howmanystudies)
...
pval_mass = Array{Union{Float64,Missing}}(missing, howmanystudies)
...
# open an empty vector to hold simulated optimal diet information
SE_sim_opt = zeros(Float64, howmanystudies)
Rich_sim_opt = zeros(Float64, howmanystudies)
SE_sim_rand = zeros(Float64, howmanystudies)
Rich_sim_rand = zeros(Float64, howmanystudies)
Sim_slopes_rand = zeros(Float64, howmanystudies)
Sim_slopes_opt = zeros(Float64, howmanystudies)
...
NE_rate = zeros(Float64, howmanystudies)
...
# ==========================================================================================
include("simulate_optimal_diet.jl")
include("simulate_random_diet.jl")
...
for gg = 1:length(candidate_h) # in this loop, run through different guess for h
    ...
    for i = 1:howmanystudies # in this loop, run through all the different diets
        ...
        if sum(df_diet.CountInt) > 20
            ...
            #= do a scan for prey families in each diet
            for w = 1:size(sums_by_family,1) # run through the families
                if sum(occursin.(sums_by_family.PreyFamily[w], df_diet.PreyFamily)) > 0 # check if the family is in the diet
                    family_matrix[i,w] = 1 # indicate this with a 1 in the family column
                end
            end=#
            ...
            # APPROACH 2 -- do regressions between ranks
                ...
                #rank_diffs = df_ranked.CountRank .- df_ranked.DensRank            
                #scatter!(ax_mass_rank,df_ranked.Prey_mass,rank_diffs, color = :black, alpha=0.4)
                ...
                #lsline_count = lm(@formula(Frequencies ~ Rel_density), df_diet) # regression
                #new_x_for_lsline = DataFrame(Rel_density = [0:0.01:maximum(df_diet.Rel_density);]) # make new x
                #df_predict_y = predict(lsline_count,new_x_for_lsline) # make new y
                #lines!(ax_dens,new_x_for_lsline.Rel_density,df_predict_y) # plot line

                #count_r2 = r2(lsline_count)
                #cor(df_ranked.DensRank,df_ranked.CountRank)
                #corspearman(df_ranked.DensRank,df_ranked.CountRank)
                #sqrt(count_r2)
                # calculate the RMA slope
                #slope_dens = coef(lsline_count)[2] / sqrt(count_r2)
                #slope_dens = cov(df_ranked.DensRank,df_ranked.CountRank)
                #int_dens = df_ranked.CountRank[1] - slope_dens * df_ranked.DensRank[1]
                #pred_line_dens = slope_dens .* new_x_for_lsline.DensRank .+ int_dens
                #lines!(ax_dens,new_x_for_lsline.DensRank,pred_line_dens) # plot line

                # make and plot a least-squares line for the count-body mass rank relationship
                #lsline_mass = lm(@formula(CountRank ~ MassRank), df_ranked) # regression
                #new_x_for_lsline = DataFrame(MassRank = 1:1:maximum(df_ranked.MassRank)) # make new x
                #df_predict_y = predict(lsline_mass,new_x_for_lsline) # make new ylabel
                #lines!(ax_mass,new_x_for_lsline.MassRank,df_predict_y) # plot line

                dens_slopes[i] = coef(lsline_count)[2] # grab the slope of the model
                #mass_slopes[i] = coef(lsline_mass)[2] # grab the slope of the model

                numb_prey[i] = sum(df_ranked.CountInt)
                numb_prey_spp[i] = size(df_ranked,1)
                #scatter!(ax_dens,df_ranked.DensRank,df_ranked.CountRank)

                pval_dens[i] = coeftable(lsline_count).cols[4][2]
                #pval_mass[i] = coeftable(lsline_mass).cols[4][2]
            ...
            #SE_sim_opt[i], Rich_sim_opt[i], Sim_slopes_opt[i] = simulate_optimal_diet(baow_a,baow_h,sum_R,df_ranked2,prey_max,num_simulations)
            #SE_sim_rand[i], Rich_sim_rand[i], Sim_slopes_rand[i] = simulate_random_diet(baow_a,baow_h,sum_R,df_ranked2,prey_max,num_simulations)

            println(i)
            if !ismissing(p3)
                # expected prey mass gain rate per foraging period (assume 8 hours)
                # two ways - old way was do the whole diet
                # keep this for the overall diet
                NE_rate[i] = 3 * mean(df_ranked.PreyMass, weights(df_ranked.CountInt .+ 0)) * p3 # whole diet
                # net energy return for all the candidate prey       
                # NE_prey = df_ranked.PreyMass ./ baow_h            
                #scatter!(ax_net,repeat([NE_rate[i]],length(NE_prey)), NE_prey, color = colorgrad[8], alpha=0.4)
                #scatter!(ax_net,NE_rate[i], NE_rate[i], marker = :diamond, color = :black)    
                ...
                if length(df_ranked.PreyMass) > 1
                    ...
                    for zz = 1:length(df_ranked.PreyMass)
                        NE_rate_discounted = 3 * (mean(df_ranked.PreyMass, weights(df_ranked.Frequencies .+ 0)) -
...
# ===================================================
df_meta[!, :mass_slopes] = mass_slopes
...
df_meta[!, :Simulated_opt_SE] = SE_sim_opt
df_meta[!, :Simulated_opt_richness] = Rich_sim_opt
df_meta[!, :Simulated_rand_SE] = SE_sim_rand
df_meta[!, :Simulated_rand_richness] = Rich_sim_rand
df_meta[!, :Sim_slopes_rand] = Sim_slopes_rand
df_meta[!, :Sim_slopes_opt] = Sim_slopes_opt
...
# ====================================================================
# Plot h against the proportion of diet matching the rule
# ====================================================================
...
df_meta_subset1 = dropmissing(df_meta,:PreyPerPellet)
    df_meta_subset1 = df_meta_subset1[findall(df_meta_subset1.numb_prey_spp .!= 0),:] # drop rows with 0 body mass
    df_meta_subset1 = df_meta_subset1[findall(df_meta_subset1.NumPos .!= 0),:] # drop rows with 0 body mass
...
model_1 = fit(MixedModel, @formula(NumPos ~ log(sum_R) + log(numb_prey_spp) + log(NE_intake) + FreqBirds + RERichness + (1|ecozone)), df_meta_subset1)
...
# ===================================================
# coming back to the net energy gain figure
# ===================================================
f_net_energy
...
# ===================================================
# plot the a's against the FoRAGE allometry
# ===================================================
df_FoRAGE = CSV.read("FoRAGE_db_V5_Dec_20_2024_sources_and_meta.csv",DataFrame) # read in density data
    PredMass = df_FoRAGE.PredatorMass
    FoRAGE_a = df_FoRAGE.Fitted_a

    baow_observed_a = select(df_meta,:baow_a) # pull the a's from df_meta
    baow_observed_a = baow_observed_a[findall(baow_observed_a.baow_a .!= 0),:] # drop rows with 0 for a
    baow_mean_a = mean(baow_observed_a.baow_a)*1e5

f_allometry_a = Figure()
    ax_a = Axis(f_allometry_a[1, 1], xlabel = "Predator body mass (mg)", ylabel = "Space clearance rate (mg per pred per day)", yscale = log10, xscale = log10)
    a1 = scatter!(ax_a,PredMass, FoRAGE_a, color = colorgrad[8], alpha=0.4)
    #hm = scatter!(ax_a, 6e5, baow_mean_a, color = :red, marker = :diamond)
    a2 = scatter!(ax_a, repeat([6e5],length(baow_observed_a.baow_a)), baow_observed_a.baow_a[:].*1e5, color = :black, alpha=0.4)
    Legend(f_allometry_a[1, 2], [a1, a2], ["FoRAGE", "Barn Owls"])
    save("Allometry_of_a.png",f_allometry_a)


df_meta_subset1 = dropmissing(df_meta,:PreyPerPellet)
df_meta_subset1 = df_meta_subset1[findall(df_meta_subset1.sum_R .!= 0),:] # drop rows with 0 body mass

f_fun_response = Figure()
    ax_fr = Axis(f_fun_response[1, 1], xlabel = "Small mammal density", ylabel = "Foraging rate (prey per pred per day)")

    for qq = 1:size(df_meta_subset1,1)
        xx = range(0, maximum(df_meta_subset1.sum_R), length=10)
        yy = df_meta_subset1.baow_a[qq] .* xx ./ (1 .+ 0.02 .* df_meta_subset1.baow_a[qq] .* xx)
        lines!(ax_fr,xx,yy,color = :gray, linewidth=1)

    end

    a1 = scatter!(ax_fr,df_meta_subset1.sum_R, df_meta_subset1.PreyPerPellet, color = colorgrad[8], alpha=0.4)

    save("BAOW_fun_response.png",f_fun_response)
...
model_1 = fit(MixedModel, @formula(dens_slopes ~ sum_R + numb_prey_spp + (1|ecozone)), df_meta)
...
model_1 = fit(MixedModel, @formula(dens_slopes ~ FreqBirds + (1|ecozone)), df_meta)
model_1 = fit(MixedModel, @formula(dens_slopes ~ RaptorRichness + (1|ecozone)), df_meta)
...
density_model1 = lm(@formula(baow_a ~ numb_prey_spp), df_meta)
density_model1 = lm(@formula(baow_a ~ RERichness), df_meta)
density_model1 = lm(@formula(baow_a ~ mass_range), df_meta)
density_model1 = lm(@formula(baow_a ~ largest_prey_mass), df_meta)
density_model1 = lm(@formula(baow_a ~ smallest_prey_mass), df_meta)

density_model1 = lm(@formula(baow_a ~ RERichness * numb_prey_spp), df_meta)
r2(density_model1)
